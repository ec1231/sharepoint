...
class Sharepoint {
    ...
    constructor(siteUrl) {
        ...
        if (!authScope) {
            throw new Error(
                "SHAREPOINT_AUTH_SCOPE environment variable has not been set"
            )
        }

        if (
            !(
                authScope.toLowerCase().startsWith("https://") &&
                authScope.toLowerCase().endsWith(".sharepoint.com/.default")
            )
        ) {
            throw new Error(
                'SHAREPOINT_AUTH_SCOPE environment variable value is not valid - it must begin with "https://" and end with ".sharepoint.com/.default"'
            )
        }
        ...
        if (!clientId) {
            throw new Error(
                "SHAREPOINT_CLIENT_ID environment variable has not been set"
            )
        }
        ...
        if (!tenantId) {
            throw new Error(
                "SHAREPOINT_TENANT_ID environment variable has not been set"
            )
        }
        ...
        if (!certPassphrase) {
            throw new Error(
                "SHAREPOINT_CERT_PASSPHRASE environment variable has not been set"
            )
        }
        ...
        if (!certFingerprint) {
            throw new Error(
                "SHAREPOINT_CERT_FINGERPRINT environment variable has not been set"
            )
        }

        if (certFingerprint.length !== 40) {
            throw new Error(
                "SHAREPOINT_CERT_FINGERPRINT environment variable value is not valid - it must be exactly 40 characters in length"
            )
        }

        const certPrivateKeyFileFile =
            process.env.SHAREPOINT_CERT_PRIVATE_KEY_FILE
        if (!certPrivateKeyFileFile) {
            throw new Error(
                "SHAREPOINT_CERT_PRIVATE_KEY_FILE environment variable has not been set"
            )
        }

        if (
            !(
                fs.existsSync(certPrivateKeyFileFile) &&
                fs.lstatSync(certPrivateKeyFileFile).isFile()
            )
        ) {
            throw new Error(
                `specified sharepoint certificate private key file ('${certPrivateKeyFileFile}') does not exist`
            )
        }
        ...
        this.debug =
            this.debug &&
            (this.debug.toUpperCase() === "Y" ||
                this.debug.toUpperCase() === "YES" ||
                this.debug.toUpperCase() === "TRUE")
        if (this.debug) {
    ...
    async getWebEndpoint() {
        ...
        let response
        try {
            response = await axios.get(`${this.siteUrl}/_api/web`, {
                headers: {
        ...
        this.encodedBaseUrl = encodeURIComponent(
            response.data.d.ServerRelativeUrl
        )
    }
    ...
    async getContents(path) {
        ...
        const get = (type) => {
            try {
                return axios.get(
                    `${this.siteUrl}/_api/web/GetFolderByServerRelativeUrl('${
                        this.encodedBaseUrl
                ...
                logAxiosError(
                    this.debug,
                    err,
                    `Failed to get folder contents (type: ${type})`
                )
            }
        }
    ...
    async checkFolderExists(path) {
        ...
        try {
            const res = await axios.get(
                `${this.siteUrl}/_api/web/GetFolderByServerRelativeUrl('${
                    this.encodedBaseUrl
                }${encodeURIComponent(path)}')/Exists`,
                {
                    headers: {
                        Authorization: `Bearer ${this.accessToken}`,
                        Accept: "application/json;odata=verbose",
    ...
    async deleteFolder(path) {
        ...
        try {
            await axios({
                method: "post",
                url: `${this.siteUrl}/_api/web/GetFolderByServerRelativeUrl('${
                    this.encodedBaseUrl
                }${encodeURIComponent(path)}')`,
                headers: {
                    Authorization: `Bearer ${this.accessToken}`,
    ...
    async createFile(options) {
        ...
        try {
            await axios({
                method: "post",
                url: `${this.siteUrl}/_api/web/GetFolderByServerRelativeUrl('${path}')/Files/add(url='${fileName}', overwrite=true)`,
                data,
                headers: {
                    Accept: "application/json;odata=verbose",
                    Authorization: `Bearer ${this.accessToken}`,
    ...
    async deleteFile(options) {
        ...
        try {
            await axios({
                method: "post",
                url: `${this.siteUrl}/_api/web/GetFileByServerRelativeUrl('${path}/${fileName}')`,
                headers: {
                    Authorization: `Bearer ${this.accessToken}`,
                    "X-RequestDigest": formDigestValue,
                    "X-HTTP-Method": "DELETE",
    ...
    async downloadFile(path, savePath = null) {
        ...
        await fetch(
            `${
                this.siteUrl
            }/_layouts/15/download.aspx?sourceUrl=${encodeURIComponent(path)}`,
            {
                headers: {
                    Authorization: `Bearer ${this.accessToken}`,
                    Accept: "*/*",
...
function checkHeaders(accessToken) {
    if (!accessToken) {
        throw new Error(
            "Access token not available - please authenticate() prior to calling this function"
        )
    }
}
